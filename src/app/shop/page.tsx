...
import { Package, Archive, Truck, ShoppingCart, Plus, Minus, CheckCircle2, ArrowRight, X, Trash2, ShoppingBag, User, Mail, Phone, AlertCircle } from 'lucide-react';
...
const products = [
  { id: 1, name: "Small Shipping Box", price: "£2.50", desc: "Reinforced cardboard, ideal for books and small heavy items.", icon: Package, badge: "Most Popular" },
  { id: 2, name: "Standard Shipping Box", price: "£4.50", desc: "Durably built for general household items and clothes.", icon: Package },
  { id: 3, name: "Large Shipping Box", price: "£6.00", desc: "Perfect for larger, bulky items. Double-walled for strength.", icon: Package },
  { id: 4, name: "Shipping Barrel (Tanque)", price: "£35.00", desc: "Standard 220L blue barrel for shipments to the DR. Includes lid.", icon: Archive, badge: "DR Specialist" },
  { id: 5, name: "Heavy Duty Packing Tape", price: "£3.00", desc: "Industrial strength tape to ensure your items stay sealed.", icon: Package },
  { id: 6, name: "Bubble Wrap (10m)", price: "£8.50", desc: "Essential for protecting fragile items and electronics.", icon: ShieldIcon }
];
...
export default function ShopPage() {
  ...
  const cartCount = useMemo(() => Object.values(cart).reduce((acc, qty) => acc + qty, 0), [cart]);

  const cartTotal = useMemo(() => {
    return Object.entries(cart).reduce((acc, [id, qty]) => {
      const product = products.find(p => p.id === Number(id));
      if (!product) return acc;
      return acc + parseFloat(product.price.replace('£', '')) * qty;
    }, 0);
  }, [cart]);
  ...
  const deleteItem = (id: number) => setCart(prev => { const next = { ...prev }; delete next[id]; return next; });
  ...
  const handleConfirmRequest = async () => {
    ...
    const items = Object.entries(cart).map(([id, qty]) => {
      ...
      return { name: p?.name || id, qty, price: p?.price || '' };
    });

    await supabase.from('material_orders').insert({
      id: `MO-${Date.now().toString().slice(-6)}`,
  ...
  return (
    <main className="min-h-screen pt-24 pb-20 bg-slate-50">
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => setIsCartOpen(true)} className="w-16 h-16 bg-[var(--brand-orange)] rounded-2xl flex items-center justify-center text-white shadow-xl shadow-orange-500/20 hover:scale-110 active:scale-95 transition-all">
        ...
        <div className="bg-[var(--brand-blue)] p-8 rounded-[2.5rem] mb-16 text-white flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <button
            ...
            className="bg-white text-[var(--brand-blue)] px-8 py-4 rounded-xl font-extrabold hover:bg-[var(--brand-orange)] hover:text-white transition-all shadow-xl"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((item) => (
            <motion.div key={item.id} whileHover={{ y: -5 }} className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm flex flex-col h-full group transition-all">
              ...
              <div className="flex items-center justify-between mt-auto pt-8 border-t border-slate-50">
                ...
                <div className="flex items-center gap-1">
                  {cart[item.id] > 0 ? (
                    <>
                      <button onClick={() => removeFromCart(item.id)} className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center hover:bg-slate-100 transition-colors"><Minus className="w-4 h-4 text-slate-400" /></button>
                      <span className="w-8 text-center font-bold text-[var(--brand-blue)]">{cart[item.id]}</span>
                      <button onClick={() => addToCart(item.id)} className="w-10 h-10 rounded-xl bg-[var(--brand-blue)] flex items-center justify-center text-white hover:bg-blue-800 transition-colors"><Plus className="w-4 h-4" /></button>
                    </>
                  ) : (
                    <button onClick={() => addToCart(item.id)} className="bg-slate-900 text-white px-6 py-2 rounded-xl text-xs font-bold hover:bg-[var(--brand-orange)] transition-colors flex items-center gap-2">
      ...
      <AnimatePresence>
        {isCartOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsCartOpen(false)} className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[60]" />
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white z-[70] shadow-2xl flex flex-col">
              ...
              <div className="flex-grow overflow-y-auto p-8 space-y-6">
                {Object.entries(cart).length === 0 ? (
                  ...
                  return (
                    <div key={id} className="flex gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100">
              ...
              <div className="p-8 bg-slate-50 border-t border-slate-100 space-y-4">
                ...
                <button
                  ...
                  onClick={() => { setFormError(''); setShowCheckoutForm(true); }}
      ...
      <AnimatePresence>
        {showCheckoutForm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 italic uppercase tracking-tighter">Your Details</h3>
                  <p className="text-xs text-slate-400 font-bold mt-1">So we can arrange delivery with your next collection</p>
                </div>
                <button onClick={() => setShowCheckoutForm(false)} className="p-3 hover:bg-slate-100 rounded-2xl transition-colors">
                  <X className="w-5 h-5 text-slate-400" />
                </button>
              </div>
              ...
              <AnimatePresence>
                {formError && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-red-50 text-red-500 p-4 rounded-xl text-xs font-bold flex items-center gap-3 border border-red-100">
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 outline-none text-sm font-bold focus:border-blue-300 transition-colors"
                    value={contact.name}
              ...
              <div className="bg-slate-50 rounded-2xl p-4 space-y-2">
                ...
                {Object.entries(cart).map(([id, qty]) => {
                  ...
                  if (!p) return null;
                  return (
                    <div key={id} className="flex justify-between text-xs font-bold text-slate-600">
                      <span>{p.name} × {qty}</span>
                      <span>£{(parseFloat(p.price.replace('£', '')) * qty).toFixed(2)}</span>
                    </div>
                  );
                })}
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowCheckoutForm(false)} className="flex-1 py-4 rounded-2xl bg-slate-100 text-slate-500 text-sm font-black uppercase tracking-widest hover:bg-slate-200 transition-colors">
                  Back
                </button>
                <button
                  onClick={handleConfirmRequest}
                  disabled={submitting}
                  className="flex-1 py-4 rounded-2xl bg-[var(--brand-blue)] text-white text-sm font-black uppercase tracking-widest hover:bg-[var(--brand-orange)] transition-colors disabled:opacity-60"
                >
      ...
      <AnimatePresence>
        {isCheckedOut && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[var(--brand-blue)]/95 flex items-center justify-center p-6 text-white text-center">
